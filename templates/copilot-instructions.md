...
이 프로젝트는 공유 AI Rules를 따른다. 작업 전에 아래 규칙을 읽고, 충돌이 있으면 이 프로젝트 루트의 지시사항을 우선한다.
...
- [FSD 아키텍처](../.ai/rules/fsd/fsd-architecture.md)
- [FSD 관심사 분리](../.ai/rules/fsd/separation-of-concerns.md)
...
- 이 프로젝트에 체크아웃되지 않은 규칙 링크는 제거한다.
- 규칙 본문을 이 파일에 복사하지 않는다.
- 규칙 변경은 `.ai/rules` 서브모듈이 가리키는 원본 저장소에서 수정한다.
